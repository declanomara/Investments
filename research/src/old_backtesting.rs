...
impl Row {
    pub fn to_string(&self) -> String {
        ...
        for metric in self.data.iter() {
            match metric {
                BacktestMetric::Float(value) => string.push_str(&format!("{:.5},", value)),
                BacktestMetric::Int(value) => string.push_str(&format!("{},", value)),
                BacktestMetric::String(value) => string.push_str(&format!("{},", value)),
                BacktestMetric::Bool(value) => string.push_str(&format!("{},", value)),
            }
        }
        string
    }
